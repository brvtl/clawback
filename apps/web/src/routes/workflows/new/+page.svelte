<script lang="ts">
  ...
  async function createWorkflow() {
    ...
    try {
      ...
      await goto(`/workflows/${response.workflow.id}`);
    } catch (e) {
...
<svelte:head>
  <title>New Workflow | Clawback</title>
</svelte:head>
...
<div class="p-6 max-w-4xl mx-auto">
  ...
  <form on:submit|preventDefault={createWorkflow} class="space-y-6">
    <!-- Basic Info -->
...
5. Comment on the GitHub issue with links to all created resources"
    ...
    <div class="flex items-center justify-end gap-4">
      <a href="/workflows" class="px-6 py-2 text-gray-400 hover:text-white transition-colors">
        Cancel
      </a>
      <button
        type="submit"
